...
def test_pagination_workspaces_and_boards_live(client_live):
    # Create 10 temporary workspaces with a unique prefix
    ...
    try:
        ...
        # Iterate over all workspaces in pages of 3 and collect only ours
        seen_ids: list[str] = []
        for ws in client_live.iter_workspaces(page_size=3, fields=("id", "name")):
        ...
        for bd in client_live.iter_boards(page_size=3, workspace_id=wbid, fields=("id", "name")):
